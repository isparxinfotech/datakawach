...
export class UploadComponent implements OnInit, OnDestroy {
  ...
  async onCreateSchedule() {
    ...
    const url = 'https://datakavach.com/cloud/schedule';
  ...
  async triggerManualBackup(scheduleId: number) {
    ...
    const url = 'https://datakavach.com/cloud/trigger-backup';
    const body = new FormData();
  ...
  async loadBackupSchedules() {
    ...
    this.message = '';
    const url = `https://datakavach.com/cloud/schedules?username=${encodeURIComponent(this.userSessionDetails.username)}`;
  ...
  async loadRootFolders() {
    ...
    const url = isRetentionNeeded
      ? `https://datakavach.com/cloud/user-folders?username=${encodeURIComponent(this.userSessionDetails.username)}`
      : `https://datakavach.com/cloud/folders?username=${encodeURIComponent(this.userSessionDetails.username)}`;
  ...
  async loadFolderContents(folderPath: string, nextLink?: string) {
    ...
    this.message = '';
    let url = `https://datakavach.com/cloud/folder-contents?username=${encodeURIComponent(this.userSessionDetails.username)}&folderPath=${encodeURIComponent(folderPath)}`;
    if (nextLink) {
      url = nextLink;
    }
    ...
    if (this.uploadType === 'file') {
      ...
      for (let i = 0; i < this.selectedFiles.length; i++) {
        ...
        for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
          ...
          const url = `https://datakavach.com/cloud/upload/${encodeURIComponent(rawFileName)}`;
